...
export class UserHelper {
  ...
  public async checkRedisSession(): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to check Redis session');
      }

      const data = await response.json()
      return data?.exists === true
    } catch (error) {
      console.error('[UserHelper] Error checking Redis session:', error);
      return false
    }
  }
  ...
  public async testRedisSet(key: string, value: string): Promise<boolean> {
    try {
      ...
      console.error('[UserHelper] Error testing Redis set:', error);
      return false
    }
  }
  ...
  public async testRedisGet(key: string): Promise<string | null> {
    try {
      ...
      console.error('[UserHelper] Error testing Redis get:', error)
      return null
    }
  }
  ...
  public async testRedisDelete(key: string): Promise<boolean> {
    try {
      ...
      console.error('[UserHelper] Error testing Redis delete:', error)
      return false;
    }
  }
}
